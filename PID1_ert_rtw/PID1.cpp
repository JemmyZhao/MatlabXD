...
// C/C++ source code generated on : Mon Jun 26 11:06:25 2017
//
...
extern void PID1_derivatives();

//
// This function updates continuous states using the ODE2 fixed-step
// solver algorithm
//
void PID1ModelClass::rt_ertODEUpdateContinuousStates(RTWSolverInfo *si )
{
...
// Model step function
void PID1ModelClass::step()
{
  real_T rtb_Sum_c;
  real_T rtb_ProportionalGain_i;
  real_T rtb_ProportionalGain_bp;
  ...
  rtb_Sum_c = 0.017453292519943295 * rtU.yaw_in - rtU.yaw_fb;
  ...
  rtDW.FilterCoefficient = (10.0 * rtb_Sum_c - rtX.Filter_CSTATE) * 20.0;
  ...
  rtb_Sum_c = ((70.0 * rtb_Sum_c + rtX.Integrator_CSTATE) +
               rtDW.FilterCoefficient) - rtU.yaw_vel_fb;
  ...
  rtDW.FilterCoefficient_a = (100.0 * rtb_Sum_c - rtX.Filter_CSTATE_o) * 20.0;

  // Gain: '<S2>/Integral Gain'
  rtDW.IntegralGain = 0.0;

  // Gain: '<S2>/Proportional Gain'
  rtb_ProportionalGain_i = 1500.0 * rtb_Sum_c;
  ...
  rtb_Sum_c = 0.017453292519943295 * rtU.roll_in - rtU.roll_fb;
  ...
  rtDW.FilterCoefficient_m = (10.0 * rtb_Sum_c - rtX.Filter_CSTATE_p) * 20.0;
  ...
  rtb_Sum_c = ((70.0 * rtb_Sum_c + rtX.Integrator_CSTATE_l1) +
               rtDW.FilterCoefficient_m) - rtU.roll_vel_fb;
  ...
  rtDW.FilterCoefficient_k = (200.0 * rtb_Sum_c - rtX.Filter_CSTATE_k) * 100.0;

  // Gain: '<S3>/Integral Gain'
  rtDW.IntegralGain_h = 0.0;

  // Gain: '<S3>/Proportional Gain'
  rtb_ProportionalGain_bp = 1000.0 * rtb_Sum_c;
  ...
  //   Gain: '<S1>/rad2deg5'
  //   Inport: '<Root>/pitch_fb'
  //   Inport: '<Root>/pitch_in'

  rtb_Sum_c = 0.017453292519943295 * rtU.pitch_in - rtU.pitch_fb;
  ...
  rtDW.FilterCoefficient_e = (10.0 * rtb_Sum_c - rtX.Filter_CSTATE_kn) * 20.0;
  ...
  rtb_Sum_c = ((70.0 * rtb_Sum_c + rtX.Integrator_CSTATE_a) +
               rtDW.FilterCoefficient_e) - rtU.pitch_vel_fb;
  ...
  rtDW.FilterCoefficient_o = (200.0 * rtb_Sum_c - rtX.Filter_CSTATE_a) * 100.0;
  ...
  rtb_ProportionalGain_i = (rtb_ProportionalGain_i + rtX.Integrator_CSTATE_l) +
    rtDW.FilterCoefficient_a;

  // Saturate: '<S2>/Saturate'
  if (rtb_ProportionalGain_i > 1000.0) {
    // Outport: '<Root>/yaw_out'
    rtY.yaw_out = 1000.0;
  } else if (rtb_ProportionalGain_i < -1000.0) {
    // Outport: '<Root>/yaw_out'
    rtY.yaw_out = -1000.0;
  } else {
    // Outport: '<Root>/yaw_out'
    rtY.yaw_out = rtb_ProportionalGain_i;
  }
  ...
  rtb_ProportionalGain_i = (rtb_ProportionalGain_bp + rtX.Integrator_CSTATE_i) +
  ...
  if (rtb_ProportionalGain_i > 1000.0) {
    ...
    // Outport: '<Root>/roll_out'
    rtY.roll_out = rtb_ProportionalGain_i;
  }
  ...
  rtb_ProportionalGain_i = (1000.0 * rtb_Sum_c + rtX.Integrator_CSTATE_id) +
  ...
  if (rtb_ProportionalGain_i > 1000.0) {
    ...
    // Outport: '<Root>/pitch_out'
    rtY.pitch_out = rtb_ProportionalGain_i;
  }
...
void PID1ModelClass::PID1_derivatives()
{
  XDot *_rtXdot;
...
void PID1ModelClass::initialize()
{
  // Registration code
  {
    // Setup solver object
...
RT_MODEL * PID1ModelClass::getRTM()
{
  return (&rtM);
}
