...
void svd(const float A[36], float U[36], float s[6], float V[36])
{
  ...
  for (q = 0; q < 5; q++) {
    ...
    if (ztest0 > 0.0F) {
      ...
      if (std::abs(b_s[q]) >= 9.86076132E-32F) {
        ztest0 = 1.0F / b_s[q];
        i = (iter - q) + 6;
  ...
  snorm = 0.0F;
  for (qp1jj = 0; qp1jj < 6; qp1jj++) {
    ztest0 = std::abs(b_s[qp1jj]);
    ztest = std::abs(e[qp1jj]);
  ...
  while ((m + 2 > 0) && (!(iter >= 75))) {
    ...
    do {
      ...
      if (qp1jj + 1 == 0) {
        ...
        ztest0 = std::abs(e[qp1jj]);
        if ((ztest0 <= 1.1920929E-7F * (std::abs(b_s[qp1jj]) + std::abs
    ...
    if (qp1jj + 1 == m + 1) {
      ...
      while ((!exitg2) && (i >= qp1jj + 1)) {
        ...
        if (i == qp1jj + 1) {
          ...
          ztest0 = 0.0F;
          if (i < m + 2) {
            ztest0 = std::abs(e[i - 1]);
          }

          if (i > qp1jj + 2) {
            ztest0 += std::abs(e[i - 2]);
          }
